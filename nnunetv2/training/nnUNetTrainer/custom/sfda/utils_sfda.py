...
def tensor_rot_90(x):
    if x.dim() == 4:
        ...
        return x.flip(4).transpose(3, 4)

def tensor_rot_180(x):
    if x.dim() == 4:
        return x.flip(3).flip(2)
    else:
        return x.flip(4).flip(3)

def tensor_rot_270(x):
    if x.dim() == 4:
        return x.transpose(2, 3).flip(3)
    else:
        return x.transpose(3, 4).flip(4)

def rotate_single_with_label(img, label):
    if label == 1:
        return tensor_rot_90(img)
    elif label == 2:
        return tensor_rot_180(img)
    elif label == 3:
        return tensor_rot_270(img)
    else:
        return img

# Log

class nnUNetSFDALogger(object):
    """
    ...
    def plot_progress_png(self, output_folder):
        ...
        # learning rate
        ax = ax_all[2]
        ax.plot(x_values, self.my_fantastic_logging['lrs_1'][:epoch + 1], color='b', ls='-', label="learning rate 1", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['lrs_2'][:epoch + 1], color='r', ls='-', label="learning rate 2", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['lrs_3'][:epoch + 1], color='g', ls='-', label="learning rate 3", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['lrs_4'][:epoch + 1], color='m', ls='-', label="learning rate 4", linewidth=4)
        ax.set_xlabel("epoch")
